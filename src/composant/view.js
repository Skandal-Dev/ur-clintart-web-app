...
class View extends React.Component{
    ...
    handleStop(event, data) {
        // data.x = horizontal (left), data.y = vertical (top)
        const x = Math.round(data.x);
        const y = Math.round(data.y);

        // sauvegarde propre dans le state parent
        this.props.setLeft(x);
        this.props.setTop(y);

        // (optionnel) applique tout de suite le transform arrondi sur l'élément
        const img = document.querySelector('.card-picture');
        if (img) {
            img.style.transition = 'none';
            img.style.transform = `translate(${x}px, ${y}px)`;
        }

        console.log('stop pos', x, y);
    }
    ...
    render(){
        ...
        return (
            ...
            <div id="card" className={`ur-card card-${this.props.rarity} card-bg-${this.props.rarity} card-bg-${prism} card-${prism}`} ref={this.props.refference}>
                <Draggable  onStop={this.handleStop} position={{x: this.props.left, y: this.props.top}}>
                  <img draggable="false" className="card-picture" src={this.props.img} style={{width: `${this.props.imageSize}%`}} data-src="" alt="picture"/>
                    ...
                    <div className="card-bottom">
                        ...
                        <div className="card-stars">
                            ...
                            <div className="card-ability black-t">
                                {this.props.ability}
                            </div>
                            <div className="card-bonus black-t">{this.props.bonus}</div>
